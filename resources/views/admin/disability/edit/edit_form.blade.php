...
<a href="{{route('TableDisabilityPages')}}">กลับ</a><br>
<h3 class="text-center">แก้ไขฟอร์มแบบยืนยันสิทธิผู้สูงอายุ</h3><br>

<form action="{{ route('DisabilityFormUpdate', $form->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')

    <label for="trade_condition">ขอขึ้นทะเบียน โดยเป็น:</label>
    <div>
        <input type="radio" id="trade_condition_option1" name="trade_condition" value="option1" {{ old('trade_condition', $form->disabilityTraders->first()->trade_condition ?? '') == 'option1' ? 'checked' : '' }} required>
        <label for="trade_condition_option1">บิดา - มารดา</label>
    </div>
    <div>
        <input type="radio" id="trade_condition_option2" name="trade_condition" value="option2" {{ old('trade_condition', $form->disabilityTraders->first()->trade_condition ?? '') == 'option2' ? 'checked' : '' }}>
        <label for="trade_condition_option2"> บุตร</label>
    </div>
    <div>
        <input type="radio" id="trade_condition_option3" name="trade_condition" value="option3" {{ old('trade_condition', $form->disabilityTraders->first()->trade_condition ?? '') == 'option3' ? 'checked' : '' }}>
        <label for="trade_condition_option3">สามี - ภรรยา</label>
    </div>
    <div>
        <input type="radio" id="trade_condition_option4" name="trade_condition" value="option4" {{ old('trade_condition', $form->disabilityTraders->first()->trade_condition ?? '') == 'option4' ? 'checked' : '' }}>
        <label for="trade_condition_option4"> พี่น้อง</label>
    </div>
    <div>
        <input type="radio" id="trade_condition_option5" name="trade_condition" value="option5" {{ old('trade_condition', $form->disabilityTraders->first()->trade_condition ?? '') == 'option5' ? 'checked' : '' }}>
        <label for="trade_condition_option5">ผู้ดูแลคนพิการตามระเบียบฯ</label>
    </div>

    <label for="elderly_name">ชื่อผู้มอบอำนาจ:</label>
    <input type="text" id="elderly_name" name="elderly_name" value="{{ old('elderly_name', $form->disabilityTraders->first()->elderly_name ?? '') }}" required>

    <label for="trader_citizen_id">เลขบัตรประชาชน:</label>
    <input type="text" id="trader_citizen_id" name="trader_citizen_id" value="{{ old('trader_citizen_id', $form->disabilityTraders->first()->citizen_id ?? '') }}" required>

    <label for="trader_address">ที่อยู่:</label>
    <textarea id="trader_address" name="trader_address" required>{{ old('elderly_name', $form->disabilityTraders->first()->address ?? '') }}</textarea>

    <label for="trader_phone_number">โทรศัพท์:</label>
    <input type="text" id="trader_phone_number" name="trader_phone_number" value="{{ old('trader_phone_number', $form->disabilityTraders->first()->phone_number ?? '') }}" required>
    ...
    <label for="type_of_disability">ประเภทความพิการ:</label>
    <div>
        <input type="radio" id="type_of_disability_option1" name="type_of_disability" value="option1"
        {{ old('type_of_disability', $form->type_of_disability) == 'option1' ? 'checked' : '' }}>
        <label for="type_of_disability_option1">ความพิการทางการเห็น</label>
    </div>
    <div>
        <input type="radio" id="type_of_disability_option2" name="type_of_disability" value="option2"
        {{ old('type_of_disability', $form->type_of_disability) == 'option2' ? 'checked' : '' }}>
        <label for="type_of_disability_option2">ความพิการทางสติปัญญา</label>
    </div>
    <div>
        <input type="radio" id="type_of_disability_option3" name="type_of_disability" value="option3"
        {{ old('type_of_disability', $form->type_of_disability) == 'option3' ? 'checked' : '' }}>
        <label for="type_of_disability_option3">ความพิการทางการได้ยินหรือสื่อความหมาย</label>
    </div>
    <div>
        <input type="radio" id="type_of_disability_option4" name="type_of_disability" value="option4"
        {{ old('type_of_disability', $form->type_of_disability) == 'option4' ? 'checked' : '' }}>
        <label for="type_of_disability_option4">ความพิการทางการเรียนร</label>
    </div>
    <div>
        <input type="radio" id="type_of_disability_option5" name="type_of_disability" value="option5"
        {{ old('type_of_disability', $form->type_of_disability) == 'option5' ? 'checked' : '' }}>
        <label for="type_of_disability_option5">ความพิการทางการเคลื่อนไหวหรือทางร่างกาย</label>
    </div>
    <div>
        <input type="radio" id="type_of_disability_option6" name="type_of_disability" value="option6"
        {{ old('type_of_disability', $form->type_of_disability) == 'option6' ? 'checked' : '' }}>
        <label for="type_of_disability_option6">ความพิการทางออทิสติก</label>
    </div>
    <div>
        <input type="radio" id="type_of_disability_option7" name="type_of_disability" value="option7"
        {{ old('type_of_disability', $form->type_of_disability) == 'option7' ? 'checked' : '' }}>
        <label for="type_of_disability_option7">ความพิการทางการจิตใจหรือพฤติกรรม</label>
    </div>
    ...
    <div>
        <input type="checkbox" name="welfare_type[]" id="welfare_type_aid" value="option1" {{ in_array('option1', old('welfare_type', $form->disabilityOptions->first()->welfare_type ?? [])) ? 'checked' : '' }}>
        <label for="welfare_type_aid">ไม่ได้รับเบี้ยยังชีพผู้สูงอายุ</label>
    </div>
    <div>
        <input type="checkbox" name="welfare_type[]" id="welfare_type_disability" value="option2" {{ in_array('option2', old('welfare_type', $form->disabilityOptions->first()->welfare_type ?? [])) ? 'checked' : '' }}>
        <label for="welfare_type_disability">ได้รับเงินสงเคราะห์เพื่อการยังชีพผู้ป่วยเอดส์</label>
    </div>
    <div>
        <input type="checkbox" name="welfare_type[]" id="welfare_type_disability_2" value="option3" {{ in_array('option3', old('welfare_type', $form->disabilityOptions->first()->welfare_type ?? [])) ? 'checked' : '' }}>
        <label for="welfare_type_disability_2">ได้รับเงินเบี้ยความพิการ</label>
    </div>
    <div>
        <input type="checkbox" name="welfare_type[]" id="welfare_type_relocation" value="option4" {{ in_array('option4', old('welfare_type', $form->disabilityOptions->first()->welfare_type ?? [])) ? 'checked' : '' }}>
        <label for="welfare_type_relocation">ย้ายภูมิลำเนาเข้ามาอยู่ใหม่ เมื่อ</label>
    </div>


    <div id="welfare_other_types_div" style="display: {{ in_array('option4', old('welfare_type', $form->disabilityOptions->first()->welfare_type ?? [])) ? 'block' : 'none' }};">
        <label for="welfare_other_types">รายละเอียดอื่นๆ</label>
        <input type="text" id="welfare_other_types" name="welfare_other_types" value="{{ old('welfare_other_types', $form->disabilityOptions->first()->welfare_other_types ?? '') }}" placeholder="กรอกข้อมูลเพิ่มเติม">
    </div>
    ...
    <div>
        <input type="radio" name="request_for_money_type" id="money_type_option1" value="option1" {{ old('request_for_money_type', $form->disabilityOptions->first()->request_for_money_type ?? '') == 'option1' ? 'checked' : '' }} required>
        <label for="money_type_option1">รับเงินสดด้วยตนเอง</label>
    </div>
    <div>
        <input type="radio" name="request_for_money_type" id="money_type_option2" value="option2" {{ old('request_for_money_type', $form->disabilityOptions->first()->request_for_money_type ?? '') == 'option2' ? 'checked' : '' }}>
        <label for="money_type_option2">รับเงินสดโดยบุคคลที่ได้รับมอบอํานาจจากผู้มีสิทธิ</label>
    </div>
    <div>
        <input type="radio" name="request_for_money_type" id="money_type_option3" value="option3" {{ old('request_for_money_type', $form->disabilityOptions->first()->request_for_money_type ?? '') == 'option3' ? 'checked' : '' }}>
        <label for="money_type_option3">โอนเงินเข้าบัญชีเงินฝากธนาคารในนามผู้มีสิทธิ</label>
    </div>
    <div>
        <input type="radio" name="request_for_money_type" id="money_type_option4" value="option4" {{ old('request_for_money_type', $form->disabilityOptions->first()->request_for_money_type ?? '') == 'option4' ? 'checked' : '' }}>
    ...
    <div>
        <input type="checkbox" name="bank_option" id="bank_option" value="1" onclick="toggleAccountInputs()" value="1" {{ old('bank_option', $form->disabilityBankAccounts->first()->bank_option ?? '') == '1' ? 'checked' : '' }}>
    ...
    <div class="form-group" id="bank_name_group" style="display: none;">
        ...
        <input type="text" id="bank_name" name="bank_name" value="{{ old('bank_name', $form->disabilityBankAccounts->first()->bank_name ?? '') }}">
    </div>
    ...
    <div class="form-group" id="account_number_group" style="display: none;">
        ...
        <input type="text" id="account_number" name="account_number" value="{{ old('account_number', $form->disabilityBankAccounts->first()->account_number ?? '') }}">
    </div>
    ...
    <div class="form-group" id="account_name_group" style="display: none;">
        ...
        <input type="text" id="account_name" name="account_name" value="{{ old('account_name', $form->disabilityBankAccounts->first()->account_name ?? '') }}">
    </div>
