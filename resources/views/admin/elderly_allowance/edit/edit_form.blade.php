...
<a href="{{route('TableElderlyAllowancePages')}}">กลับ</a><br>
<h3 class="text-center">แก้ไขฟอร์มแบบยืนยันสิทธิผู้สูงอายุ</h3><br>

<form action="{{ route('ElderlyAllowanceFormUpdate', $form->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div>
        <input type="checkbox" name="document_type[]" id="document_type_id_card" value="option1" {{ in_array('option1', json_decode(old('document_type', $form->personsOptions->first()->document_type ?? '[]'), true)) ? 'checked' : '' }}>
    <label for="document_type_id_card">สำเนาบัตรประจำตัวประชาชน</label>
    </div>
    <div>
        <input type="checkbox" name="document_type[]" id="document_type_house_reg" value="option2" {{ in_array('option2', json_decode(old('document_type', $form->personsOptions->first()->document_type ?? '[]'), true)) ? 'checked' : '' }}>
        <label for="document_type_house_reg">สำเนาทะเบียนบ้าน</label>
    </div>
    <div>
        <input type="checkbox" name="document_type[]" id="document_type_bank_book" value="option3" {{ in_array('option3', json_decode(old('document_type', $form->personsOptions->first()->document_type ?? '[]'), true)) ? 'checked' : '' }}>
        <label for="document_type_bank_book">สำเนาสมุดบัญชีเงินฝากธนาคาร</label>
    </div>
    <div>
        <input type="checkbox" name="document_type[]" id="document_type_auth_letter" value="option4" {{ in_array('option4', json_decode(old('document_type', $form->personsOptions->first()->document_type ?? '[]'), true)) ? 'checked' : '' }}>
